--EXEC USP_AccountDetails @BSacctId = 5000, @LinkServer = 'XEON-S8', @DB = 'PP_CI'
...
BEGIN
	...
	DECLARE @SQL NVARCHAR(MAX), @ColumnNames NVARCHAR(MAX), @ID NVARCHAR(100), @Date NVARCHAR(100)
	


	--SET @LINKSERVER = 'XEON-S8'
	--SET @DB = 'PP_JAZZ_CI'
	--SET @DB_Sec = 'PP_JAZZ_CI_Secondary'
	...
	BEGIN TRY
		...
		SET @ColumnNames = N'BP.acctId, LTRIM(RTRIM(AccountNumber)) AccountNumber, BP.UniversalUniqueID, BP.MergeInProcessPH, BP.DateAcctOpened, BP.BillingCycle, BP.LastStatementDate,
		...
		IF(@CCARD = 1)
		BEGIN
			SET @ColumnNames = N'BSAcctid, MergeActivityFlag, CP.ARTxnType, CP.TxnSource, RTRIM(CP.CMTTRANTYPE) AS CMTTRANTYPE,CP.TxnAcctId, txnsource, CP.creditplanmaster, 
