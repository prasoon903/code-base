...
(
	...
	FROM ##TempDataDisputes WHERE cmttrantype = '40'
)
...
SELECT * FROM #OriginalLM40

DROP TABLE IF EXISTS #OriginalLM43
SELECT T1.*, CP.TranID LM43TranID, CP.TransactionAmount LM43Amount, CP.TranTime LM43TxnTime, CP.PostTime LM43PostTime
INTO #OriginalLM43 
FROM #OriginalLM40 T1
JOIN CCard_primary CP WITH (NOLOCK) ON (T1.TranId = CP.RevTgt)
WHERE CP.CMTTranType = '43'

SELECT * FROM #OriginalLM43 WHERE TransactionAmount <> LM43Amount


SELECT * FROM #OriginalLM40 WHERE ClaimID IS NOT NULL

SELECT ClaimID, TransactionAmount, TranID, TranRef, RevTgt, * FROM CCard_Primary WITH (NOLOCK) WHERE TransactionLifeCycleUniqueID = 512726926
SELECT ClaimID, * FROM #OriginalLM40 WITH (NOLOCK) WHERE TransactionLifeCycleUniqueID = 9301525
SELECT * FROM ##TempDataDisputes WITH (NOLOCK) WHERE TransactionLifeCycleUniqueID = 9301525
