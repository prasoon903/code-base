...
select paymenthistprofile,* From LS_P1MARPRODDB01.ccgs_coreissue.dbo.cbreportingdetail with(nolock) where acctid = 282736
order by statementdate desc

select * From LS_P1MARPRODDB01.ccgs_coreissue.dbo.BSCBRPHPHistory with(nolock) where acctid = 282736
order by statementdate desc

select sh.currentbalance,csh.currentbalance, CycleDueDTD,* from statementheader sh with(nolock) 
join currentstatementheader csh with(nolock) on csh.acctid = sh.acctid and csh.statementdate = sh.statementdate
where sh.acctid = 282736
