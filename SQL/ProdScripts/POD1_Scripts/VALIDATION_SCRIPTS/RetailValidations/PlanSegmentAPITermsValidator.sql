DROP TABLE IF EXISTS #TempPlanInput
CREATE TABLE #TempPlanInput (PlanID INT, PlanUUID VARCHAR(64), CPM INT, OriginalLoanTerms INT, TermsReturned INT)

INSERT INTO #TempPlanInput (PlanUUID) 
SELECT PlanUUID FROM ##TempPlanInput

UPDATE TT
	SET PlanID = CPCC.acctId, CPM = CPA.Parent01AID
FROM #TempPlanInput TT
JOIN CPSgmentCreditCard CPCC WITH (NOLOCK) ON (TT.PlanUUID = CPCC.PlanUUID)
JOIN CPSgmentAccounts CPA WITH (NOLOCK) ON (CPCC.acctId = CPA.acctId)


UPDATE TT
	SET OriginalLoanTerms = CPM.EqualPayments
FROM #TempPlanInput TT
JOIN CPMAccounts CPM WITH (NOLOCK) ON (TT.CPM = CPM.acctId AND CPM.creditPlanType = '16')


--SELECT * FROM #TempPlanInput

DECLARE @CPSAcctID INT = 0
DECLARE
	@AccountID				INT,   
	@PlanID					INT,   
...
SELECT PlanID FROM #TempPlanInput WITH (NOLOCK) --WHERE Parent02AID = @BSAcctId
	...
	/**START**/
		...
		BEGIN 
			...
			BEGIN   
				...
				CREATE TABLE #TempSummaryHeaderRecords
				(
					acctId								INT,
				...
				FROM  LS_PRODDRGSDB01.ccgs_coreissue.dbo.MergeSummaryHeader MSH WITH (NOLOCK)
				JOIN LS_PRODDRGSDB01.ccgs_coreissue.dbo.MergeStatementHeader MS WITH (NOLOCK) ON (MS.acctId = MSH.parent02AID AND MSH.StatementID = MS.StatementID)
				...
				FROM   LS_PRODDRGSDB01.ccgs_coreissue.dbo.SummaryHeaderCreditCard SCC WITH (NOLOCK) 
				JOIN LS_PRODDRGSDB01.ccgs_coreissue_secondary.dbo.SummaryHeader SH WITH (NOLOCK) ON (SH.acctid = SCC.acctid AND SH.StatementID = SCC.StatementID) 
				JOIN LS_PRODDRGSDB01.ccgs_coreissue_secondary.dbo.StatementHeader S WITH (NOLOCK) ON (S.acctId = SH.parent02AID AND SH.StatementID = S.StatementID)
				JOIN LS_PRODDRGSDB01.ccgs_coreissue.dbo.StatementHeaderEx SE WITH (NOLOCK) ON (S.acctId = SE.acctId AND S.StatementID = SE.StatementID)
				...
				INSERT INTO #TempJobID
					...
					FROM LS_PRODDRGSDB01.ccgs_coreissue.dbo.SummaryHeaderCreditCard SCC WITH (NOLOCK)
					JOIN LS_PRODDRGSDB01.ccgs_coreissue_secondary.dbo.SummaryHeader SH WITH (NOLOCK) ON (SH.acctid = SCC.acctid AND SH.StatementID = SCC.StatementID) 
					JOIN LS_PRODDRGSDB01.ccgs_coreissue.dbo.CurrentSummaryHeader CSH WITH (NOLOCK) ON (SH.acctid = CSH.acctid AND SH.StatementID = CSH.StatementID) 
					JOIN #TempPlan TP ON (TP.planid = SCC.acctid)
				...
				*/
				...
				INSERT INTO #TempBilledTerms   
					SELECT 
						...
						AS OriginalDue, --ISNULL(ILP.RegularPaymentAmountForIteration, 0) AS OriginalDue,
						ILP.LastStatementDate AS ActualStatementdate
					FROM  #TempSummaryHeaderRecords TS WITH (NOLOCK)
				...
				--SELECT CC.OriginalPurchaseAmount - (CC.currentbalanceco + CPS.CurrentBalance  ) - ISNULL(TU.UnBilledAmount, 0), ISNULL(TU.UnBilledAmount, 0), CC.OriginalPurchaseAmount, CC.currentbalanceco, CPS.CurrentBalance
				--FROM #TempPlan TT
				...
				BEGIN
					...
					AND ISNULL(ILPS.ReageReversed, 0) = 0 AND (ILPS.Activity = 28 OR ILPS.Activity = 12)
				...
				IF(@OriginalLoanEndDate > @LoanEndDate)
					...
					SELECT @CountArray1 = COUNT(1) FROM #tempTermsOnPaymentApplied WITH (NOLOCK)

					IF (@CountArray1 < @LoanTerm)
					BEGIN
						CREATE TABLE #tempTermsOnPaymentAppliedExtended   
						(   
							PlanID								INT,
							TermNumber							INT,   
							LoanTerm							INT,   
						...
						SELECT @MaxLoanTerm= MAX(TermNumber) FROM #tempTermsOnPaymentApplied WITH (NOLOCK)
						--print @MaxLoanTerm
		...
		*/
		...
		UPDATE #TempPlanInput SET TermsReturned = @CalculatedLoanTerm WHERE PlanID = @CPSAcctID
	/**END**/
