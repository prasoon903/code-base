...
DROP TABLE IF EXISTS #Temp1
SELECT BP.acctId, LastStatementDate StatementDate, SRBWithInstallmentDue, CurrentBalance + CurrentBalanceCO CBCO, SystemStatus, CycleDueDTD, DisputesAmtNS 
INTO #Temp1 
...
JOIN LS_P1MARPRODDB01.ccgs_coreissue.dbo.BSegment_Secondary BS WITH (NOLOCK) ON (BS.acctId = BCC.acctid)
WHERE SRBWithInstallmentDue > CurrentBalance + CurrentBalanceCO AND SRBWithInstallmentDue > 0
AND SystemStatus <> 14 
AND CurrentBalance + CurrentBalanceCO <=  0 
AND CycleDueDTD > 1    
...
FROM LS_P1MARPRODDB01.ccgs_coreissue_Secondary.dbo.StatementHeader ST WITH (NOLOCK)
...
SELECT 
A.acctId, RTRIM(BP.AccountNumber) AccountNumber, BP.UniversalUniqueID AccountUUID, CurrentBalance, BCC.SRBWithInstallmentDue MSBAtPresent, BS.DisputesAmtNS DisputeAmtAtPresent, BP.CycleDueDTD,
...
, CASE 
	...
	ELSE 'NA'
END SystemStatusAtPresent, AmountOfPaymentsCTD, AmountOfCreditsCTD
INTO #AccountDetails
FROM AccountDetails  A
JOIN LS_P1MARPRODDB01.ccgs_coreissue.dbo.BSegment_Primary BP WITH (NOLOCK) ON (A.acctId = BP.acctId)
JOIN LS_P1MARPRODDB01.ccgs_coreissue.dbo.BSegmentCreditCard BCC WITH (NOLOCK) ON (BCC.acctId = BP.acctId)
JOIN LS_P1MARPRODDB01.ccgs_coreissue.dbo.BSegment_Secondary BS WITH (NOLOCK) ON (BCC.acctId = BS.acctId)
WHERE [Row] = 1
ORDER BY A.acctId
...
SystemStatusAtPresent,AmountOfTotalDue,CurrentDue,PastDue,[1CPD],
--[2CPD],[3CPD],[4CPD],[5CPD],[6CPD], 
AmountOfPaymentsCTD, AmountOfCreditsCTD
FROM #AccountDetails A
...
--AccountToReage
SELECT A.AccountNumber,AccountUUID,MSBAtPresent,DisputeAmtAtPresent,DelinquencyAtPresent,
...
SELECT paymentHistProfile,* FROm CBReportingDetail WITh (NOLOCk) WHERE acctId = 247924 ORDER By StatementDate DESC
...
WHERE acctId = 247924 
AND StatementDate IN ('2021-12-31 23:59:57', '2022-01-31 23:59:57')
--ORDER By StatementDate DESC
