...
; WITH CTE
AS
(SELECT *, ROW_NUMBER() OVER (PARTITION BY transactionuuid ORDER BY TranTime DESC) [RANK] FROM #CP_Auth)
SELECT T1.* 
INTO #TempNotInAuth
FROM ##TempRecords T1
LEFT JOIN CTE C ON (T1.transactionuuid = C.transactionuuid AND C.[RANK] = 1)
...
JOIN #TempNotInAuth TT ON (TT.transactionuuid = CP.UniversalUniqueID)
where 
cp.cmttrantype IN ('40')
...
DROP TABLE IF EXISTS ##TempData_ALL
SELECT * INTO ##TempData_ALL FROM #CP_Auth

INSERT INTO ##TempData_ALL
SELECT * FROM #CP1
...
; WITH CTE
AS
(SELECT *, ROW_NUMBER() OVER (PARTITION BY transactionuuid ORDER BY TranTime DESC) [RANK] FROM ##TempData_ALL)
SELECT T1.* 
INTO ##TxnNotFound
FROM ##TempRecords T1
LEFT JOIN CTE C ON (T1.transactionuuid = C.transactionuuid AND C.[RANK] = 1)
