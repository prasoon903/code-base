...
-- Query to get account which are   going  to chargeoff but  NAD is  greater than '2020-06-30 23:59:55.000'
 select    c.trantime, b.CurrentBalance,b.acctid ,b.nad,autoinitialchargeoffstartdate,systemchargeoffstatus,userchargeoffstatus,cycleduedtd,ccinhparent125aid,chargeoffdateparam,accountnumber
    from  LS_P1MARPRODDB01.ccgs_coreissue.dbo.bsegment_primary b with(nolock) join   LS_P1MARPRODDB01.ccgs_coreissue.dbo.bsegmentcreditcard  bs  with(nolock) 
	 on (b.acctid = bs.acctid )
	join LS_P1MARPRODDB01.ccgs_coreissue.dbo.commontnp c with(nolock)  on (c.acctid  = b.acctid  and c.tranid =0 and c.atid =51  )
  ...
  where  chargeoffdateparam  = '2021-11-30 23:59:55.000' and (c.trantime >='2021-11-30 23:59:57.000' or b.nad > '2021-11-30 23:59:55.000')
   ...
   select c.trantime,accountopendate,* from LS_P1MARPRODDB01.ccgs_coreissue.dbo.bsegment_primary b  with(nolock)  join  LS_P1MARPRODDB01.ccgs_coreissue.dbo.bsegmentcreditcard bcc with(nolock) on (b.acctid = bcc.acctid) 
   join LS_P1MARPRODDB01.ccgs_coreissue.dbo.commontnp c  with(nolock)  on (bcc.acctid = c.acctid  and c.atid =51 and c.tranid = 0 )
   ...
   select * from  LS_P1MARPRODDB01.ccgs_coreissue.dbo.commontnp c with(nolock) where acctid = 11498079 
...
select * from accountinfoforreport with(nolock)  where bsacctid =880088  order by businessday desc
    ...
    from LS_P1MARPRODDB01.ccgs_coreissue.dbo.bsegment_primary b with(nolock) join 
	 LS_P1MARPRODDB01.ccgs_coreissue.dbo.bsegmentcreditcard  bs  with(nolock)  on (b.acctid = bs.acctid )
where  accountnumber in (select Accountnumber  from LS_P1MARPRODDB01.ccgs_coreissue.dbo.ccard_primary cc with(nolock) 
join LS_P1MARPRODDB01.ccgs_coreissue.dbo.trans_in_acct t  with(nolock) on (cc.tranid = t.tran_id_index  and t.atid =51)  
where cmttrantype = 'RCLS')  and  systemstatus <>14  
	...
	select * from  LS_P1MARPRODDB01.ccgs_coreissue.dbo.version  order by entryid desc
	...
	'update  top(1) bsegment_primary  set  tpyblob  =  null  ,tpynad = null , tpylad = null   where    acctid  ='  +  
	 ...
	 from LS_P1MARPRODDB01.ccgs_coreissue.dbo.bsegmentcreditcard  b  with(nolock)  join   LS_P1MARPRODDB01.ccgs_coreissue.dbo.bsegment_primary  bp  with(nolock)
	 on (b.acctid = bp.acctid )
	 join  LS_P1MARPRODDB01.ccgs_coreissue.dbo.astatusaccounts a with(nolock)  on 
...
FROM AIR_0224
	...
	select tpyblob,tpynad,tpylad from LS_P1MARPRODDB01.ccgs_coreissue.dbo.bsegment_primary  with(nolock) where  acctid in (880088,1322765,1390447,2954843,1562984)
	select tpyblob,typnad,tpylad from LS_P1MARPRODDB01.ccgs_coreissue.dbo.bsegmentcreditcard  with(nolock) where  acctid in (880088,1322765,1390447,2954843,1562984)
	select tpyblob,typnad,tpylad from LS_P1MARPRODDB01.ccgs_coreissue.dbo.bsegment_primary  with(nolock) where  acctid in (880088,1322765,1390447,2954843,1562984)
...
3
    ...
    from LS_P1MARPRODDB01.ccgs_coreissue.dbo.bsegment_primary b with(nolock) join  LS_P1MARPRODDB01.ccgs_coreissue.dbo.bsegmentcreditcard  bs  with(nolock)  on (b.acctid = bs.acctid )
where B.accountnumber  in (select accountnumber from LS_P1MARPRODDB01.ccgs_coreissue.dbo.ccard_primary with(nolock) where cmttrantype = 'RCLS') and  manualinitialchargeoffreason = '0'
...
select * from ccard_primary with(nolock) where accountnumber = '1100011106647740'  order by posttime  desc
	--select * from LS_P1MARPRODDB01.ccgs_coreissue.dbo.commontnp with(nolock)  where tranid  =30731938447
  ...
  where  chargeoffdateparam <='2020-09-30 23:59:55.000'  and
   (ccinhparent125aid  in(15996,16000)  and cycleduedtd > 6)  and b.institutionid = 6981 and systemstatus  <>14   and trantime >'2020-06-30 23:59:55.000'
	...
	select  bp.acctid ,c.currentbalance,cp.cycleduedtd,cp.amountoftotaldue,bp.ccinhparent125aid  from LS_P1MARPRODDB01.ccgs_coreissue.dbo.cpsgmentaccounts c with(nolock)
	 join LS_P1MARPRODDB01.ccgs_coreissue.dbo.cpsgmentcreditcard  cp with(nolock)
	on (c.acctid = cp.acctid )
	join LS_P1MARPRODDB01.ccgs_coreissue.dbo.bsegment_primary bp with(nolock) on   (c.parent02aid = bp.acctid )
...
--UPDATE  top(1)   BSegment_Primary SET tpylad = NULL, tpyNad = NULL, tpyBlob = NULL WHERE acctId = 4407269
	...
	join LS_P1MARPRODDB01.ccgs_coreissue.dbo.commontnp c with(nolock)  on (c.acctid  = b.acctid  and c.tranid =0 and c.atid =51  )
	--join   LS_P1MARPRODDB01.ccgs_coreissue.dbo.c  TT with(nolock)  on (TT.acctid = b.acctid) where TT.jobstatus =1

	select * from LS_P1MARPRODDB01.ccgs_coreissue.dbo.statementvalidation  with(nolock) order by skey  desc
...
select  accountnumber from LS_P1MARPRODDB01.ccgs_coreissue.dbo.bsegment_primary with (nolock) where acctid = 52402
	select count(1),validationfail,ValidationDescription  from LS_P1MARPRODDB01.ccgs_coreissue.dbo.statementvalidation sl  with(nolock) 
	left  join  LS_P1MARPRODDB01.ccgs_coreissue.dbo.statementvalidationlookup  s  with(nolock)
	...
	order by skey  desc
	--52402,201817 202484
	select * from LS_P1MARPRODDB01.ccgs_coreissue.dbo.statementvalidation with (nolock) where  acctid = 201817
	--validationfail in ('q8') 
	and statementdate = '2021-08-31 23:59:57.000'
	
	select  * from  LS_P1MARPRODDB01.ccgs_coreissue.dbo.statementvalidationlookup  sl  with(nolock)
