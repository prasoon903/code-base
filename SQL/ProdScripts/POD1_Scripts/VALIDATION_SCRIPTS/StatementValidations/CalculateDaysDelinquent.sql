...
SELECT
	...
	TRY_CONVERT(DATETIME, CONVERT(VARCHAR, CAST(DateOfOriginalPaymentDueDTD AS DATE)) + ' 23:59:57'), LAD, LAPD, DeAcctActivityDate, ccinhparent125AID, 
	ActualDRPStartDate, CycleDueDTD, SystemStatus, DaysDelinquent, NoPayDaysDelinquent, FreezeDelinquency, DPDFreezeDays
...
UPDATE TBD
SET
	NoPayDaysDelinquent = DATEDIFF(DAY, DateOfOriginalPaymentDueDTD, DeAcctActivityDate) + 1 - CASE WHEN FreezeDelinquency = 1 THEN DPDFreezeDays ELSE 0 END,
	DaysDelinquent = DATEDIFF(DAY, DtOfLastDelinqCTD, DeAcctActivityDate) + 1 - CASE WHEN FreezeDelinquency = 1 THEN DPDFreezeDays ELSE 0 END
FROM #Temp_bsegment_DelinqDays TBD
--WHERE DateOfOriginalPaymentDueDTD <> DtOfLastDelinqCTD
