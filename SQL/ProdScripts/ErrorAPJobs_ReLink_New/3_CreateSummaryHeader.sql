...
DECLARE @FromStatement DATETIME
DECLARE @PreviousStatement DATETIME
DECLARE @Statement DATETIME
DECLARE @ToStatement DATETIME
DECLARE @acctID INT

--SELECT COUNT(1) FROM SummaryHeader WITH (NOLOCK)  WHERE parent02AID = 602255 AND StatementDate = @FromStatement

--SELECT * FROM AccountsOfPlanToReLink WHERE JobStatus = 1

WHILE EXISTS(SELECT TOP 1 1 FROM AccountsOfPlanToReLink WHERE JobStatus = 1)
BEGIN

	SELECT DISTINCT TOP(1) @acctID = acctID, @FromStatement = FromStatement, @ToStatement = ToStatement FROM ##AccountsOfPlanToReLink WITH(NOLOCK) WHERE JobStatus = 1

	SET @Statement = @FromStatement
	SET @PreviousStatement = DATEADD(SECOND, 86397, TRY_CAST(EOMONTH(DATEADD(MONTH, -1, @Statement)) AS VARCHAR))
	...
	BEGIN
		...
		(
			...
			WHERE StatementDate = @PreviousStatement
			AND S.parent02AID = @acctID
			EXCEPT
			SELECT S.acctID, S.parent02AID
			FROM SummaryHeader S WITH (NOLOCK)
		...
		SELECT parent02AID, acctId, @Statement StatementDate, @PreviousStatement PreviousStatementDate 
		INTO #TempData 
		FROM CTE
		...
		BEGIN TRY
			...
			BEGIN TRANSACTION
				...
				BEGIN
					UPDATE AccountsOfPlanToReLink SET JobStatus = 2 WHERE JobStatus = 1 AND acctID = @acctID
				END
					
				SET @PreviousStatement = @Statement
				SET @Statement = DATEADD(SECOND, 86397, TRY_CAST(EOMONTH(DATEADD(MONTH, 1, @Statement)) AS VARCHAR))
