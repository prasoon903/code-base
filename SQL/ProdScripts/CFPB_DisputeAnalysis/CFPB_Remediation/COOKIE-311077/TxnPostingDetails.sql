SELECT *
FROM ##CBRWriteOffPosting_COOKIE_320533
...
FROM ##CBRWriteOffPosting_COOKIE_320533 T1
JOIN BSegment_Primary B WITH (NOLOCK) ON (B.UniversalUniqueID = T1.Account_Id)
...
SELECT AccountNumber, acctID, Account_Id, Transaction_Code, ccInhparent125AID, SystemStatus, SUM(Transaction_Amount) TransactionAmount
INTO #AccountData_POD1_TxnPosting
FROM #AccountData_POD1
GROUP BY AccountNumber, acctID, Account_Id, Transaction_Code, ccInhparent125AID, SystemStatus


DROP TABLE IF EXISTS #AccountData_POD1_Txns
SELECT CP.AccountNumber, T.Account_Id, T.acctID, CP.TranID, CP.TransactionAmount, CP.TranTime, CP.PostTime, CP.PostingRef, CP.TransactionDescription, CP.ClientID
...
DROP TABLE IF EXISTS #AccountData_POD1_TCIVR
SELECT B.AccountNumber, B.acctID
INTO #AccountData_POD1_TCIVR
FROM ##CBRWriteOffPosting_COOKIE_320533 T1
JOIN BSegment_Primary B WITH (NOLOCK) ON (B.UniversalUniqueID = T1.Account_Id)
WHERE B.MergeInProcessPH IS NULL

INSERT INTO #AccountData_POD1_TCIVR
...
SELECT COUNT(1) TotalRecords FROM #AccountData_POD1_TCIVR  WITH(NOLOCK) 


SELECT ResponseMessage, COUNT(1) RecordCount
FROM TCIVRRequest T WITH (NOLOCK)
JOIN #AccountData_POD1_TCIVR A ON (T.BSAcctID = A.acctID AND T.RequestName = '11012')
WHERE T.RequestName = '11012'
AND RequestDate > '2025-05-14'
...
SELECT COUNT(1) RecorCount
FROM #AccountData_POD1 A
...
WHERE BP.CurrentBalance+BCC.CurrentBalanceCO > 0
