...
SELECT T1.*, B.DestAccountNumber AccountNumber, B.DestBSAcctId acctID, B.JobStatus, B.DestPODID, DestBSAccountUUID
...
SELECT T1.*, SRCAccountNumber, B.DestAccountNumber AccountNumber, B.DestBSAcctId acctID, B.JobStatus, B.DestPODID, B.SrcPODID
FROM ##CBRGWCPosting_COOKIE_311077 T1
JOIN MergeAccountJob B WITH (NOLOCK) ON (B.SrcBSAccountUUID = T1.Account_Id)
WHERE B.JobStatus = 'DONE'
AND B.DestPODID <> 1
...
SELECT AccountNumber, acctID, Account_Id, DestBSAccountUUID, Transaction_Code, SUM(Transaction_Amount) TransactionAmount
INTO #AccountData_POD1_Merge_TxnPosting
FROM #AccountData_POD1_Merge
GROUP BY AccountNumber, acctID, Account_Id, Transaction_Code, DestBSAccountUUID


DROP TABLE IF EXISTS #AccountData_POD1_Merge_Txns
SELECT CP.AccountNumber, T.DestBSAccountUUID AccountUUID, T.acctID, CP.TranID, CP.TransactionAmount, CP.TranTime, CP.PostTime, CP.PostingRef, CP.TransactionDescription, CP.ClientID
...
WHERE AccountNumber = '1100011116387287'
