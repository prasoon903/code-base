...
BEGIN
	...
	--, T2.APR, T2.InterestAtCycle, T2.CurrentBalance, T2.SRBWithInstallmentDue,
	--NextDQ CycleDueDTD,
	--0 AmtOfPayCurrDue,
	--0 AmountOfTotalDue,
	...
	--SELECT TOP 1 * FROM #TempDataProcessed
	...
	JOIN #TempData T3 ON (T1.acctID = T3.acctID AND T1.RN = T3.RN)
	JOIN #ProcessedData T4 ON (T1.acctID = T4.acctID AND T1.RN = T4.RN)
	WHERE T4.CycleDueDTD >= 1
...
--AND ProjectedDQ < OriginalDQ
--AND AmountOfPaymentsCTD > 0
--AND ManualStatus NOT IN (15996, 16000, 16326, 16330, 5202, 16010)
--AND PHPImpactWithStatusCheck = 'NO'
--AND RecordType = 'ImpactedRecords'