...
--Q16 - 11518718
SET @BSacctId = 11402976
--SELECT @BSacctId = acctID FROM Bsegment_Primary WITH (NOLOCK) WHERE AccountNumber = '1100011198782785'
--SELECT @BSacctId = acctID FROM Bsegment_Primary WITH (NOLOCK) WHERE UniversalUniqueID = '1efdeaaa-e304-4dc0-ac5e-4563ae4302aa'
--941c5286-1254-4130-84bc-c6feec7b3b31
--f18cc98a-0aaa-4b5e-bec7-de0f5f290378
--1efdeaaa-e304-4dc0-ac5e-4563ae4302aa
SET @LastSatementDate = '2024-11-30 23:59:57.000'
...
FROM ccgs_coreissue.dbo.BSEGMENT_PRIMARY BP WITH(NOLOCK)
	JOIN ccgs_coreissue.dbo.BSEGMENTCREDITCARD BS WITH(NOLOCK) ON (BP.ACCTID = BS.ACCTID)
	JOIN ccgs_coreissue.dbo.BSEGMENT_balances BB WITH(NOLOCK) ON (BP.ACCTID = BB.ACCTID)
	JOIN ccgs_coreissue.dbo.BSEGMENT_Secondary B2 WITH(NOLOCK) ON (BB.ACCTID = B2.ACCTID)
WHERE  bp.acctid = @BSacctId 

--SELECT 'BS_AUTH====>', DaysDelinquent, DateOfLastDelinquent, SystemStatus, CCINHPARENT125AID, * FROM ccgs_coreauth.dbo.BSegment_Primary WITH (NOLOCK) WHERE acctId = @BSacctId

--SELECT 'MergeAccountJob====>', SrcRemainingMinDue, DESTRemainingMinDue, * FROM ccgs_coreissue.dbo.MergeAccountJob WITH (NOLOCK) WHERE DestBSacctId = @BSacctId OR SrcBSacctId = @BSacctId 

PRINT 'CPS'
SELECT 'CPS====>', CPS.ACCTID,creditplantype, PlanUUID, Parent01AID, CPC.OriginalPurchaseAmount,CPC.cycleduedtd, DisputesAmtNS,
CPC.amountoftotaldue,sbwithinstallmentdue,srbwithinstallmentdue, 
...
FROM ccgs_coreissue.dbo.CPSGMENTACCOUNTS CPS WITH(NOLOCK)
	JOIN ccgs_coreissue.dbo.CPSGMENTCREDITCARD CPC WITH(NOLOCK) ON (CPS.ACCTID = CPC.ACCTID)
...
PRINT 'Statement'
	...
	FROM ccgs_coreissue_secondary.dbo.StatementHeader SH WITH (NOLOCK)
	WHERE SH.acctID = @BSacctId AND StatementDate = @LastSatementDate
	ORDER BY StatementDate DESC


	SELECT 'Summary====>',SH.acctId, APR, SH.StatementID, BeginningBalance, 
	amountofdebitsctd,amountofcreditsctd, amountofdebitsrevctd, amountofcreditsrevctd,CurrentBalanceCO,
	CreditBalanceMovement, SH.decurrentbalance_trantime_ps, SH.StatementDate, CreditPlanType, DisputesAmtNS, SHCC.CycleDueDTD, SH.CurrentBalance, CurrentBalanceCO,Principal, PrincipalCO, OriginalPurchaseAmount, 
	...
	FROM ccgs_coreissue_secondary.dbo.SummaryHeader SH WITH (NOLOCK)
	JOIN ccgs_coreissue.dbo.SummaryHeaderCreditCard SHCC WITH (NOLOCK) ON (SH.acctId = SHCC.acctId AND SH.StatementID = SHCC.StatementID)
...
PRINT 'EOD'
	...
	FROM ccgs_coreissue_secondary.dbo.AccountInfoForReport WITH (NOLOCK) 
	WHERE BusinessDay = @BusinessDay
	AND BSAcctId = @BSAcctId

	DECLARE db_Cursor CURSOR FOR
	SELECT acctId FROM ccgs_coreissue.dbo.CPSgmentAccounts WITH (NOLOCK) WHERE Parent02AID = @BSAcctId
	...
	BEGIN
		...
		FROM ccgs_coreissue_secondary.dbo.PlanInfoForReport WITH (NOLOCK) 
		WHERE BusinessDay = @BusinessDay
