...
JOIN BSegment_Secondary BS WITH (NOLOCK) ON (BS.acctId = BCC.acctid)
WHERE SRBWithInstallmentDue > CurrentBalance + CurrentBalanceCO AND SRBWithInstallmentDue > 0
AND SystemStatus <> 14 
AND CurrentBalance + CurrentBalanceCO <=  0 
AND CycleDueDTD > 1    
...
SystemStatusAtPresent,AmountOfTotalDue,CurrentDue,PastDue,[1CPD],[2CPD],[3CPD],[4CPD],[5CPD],[6CPD], AmountOfPaymentsCTD, AmountOfCreditsCTD
FROM #AccountDetails A
...
SELECT 0 TypeOfManualReage, A.AccountNumber,AmountOfTotalDue TotalDueMethod
FROM #AccountDetails A
WHERE CycleDueDTD >= 2
