...
SELECT RTRIM(BP.AccountNumber) SourceAccountNumber, BP.MergeInProcessPH, TransactionLifeCycleUniqueID, TransactionAmount, BP.DateAcctOpened DateAcctOpened_DEST
FROM #LM40ToPost TT
...
SELECT DestAccountNumber, * FROM LS_PRODDRGSDB01.ccgs_coreissue.dbo.MergeAccountJob MA WITH (NOLOCK) WHERE SrcAccountNumber = '1100011120357524'
...
MA.EndTime MergeTime, ROW_NUMBER() OVER(PARTITION BY DestAccountNumber ORDER BY T1.TranTime) RowCountData
--INTO #TempJobsToPost
FROM #LM40ToPost t1
LEFT JOIN LS_PRODDRGSDB01.ccgs_coreissue.dbo.MergeAccountJob MA WITH (NOLOCK) ON (T1.AccountNumber = MA.SRCAccountNumber)
LEFT JOIN LS_PRODDRGSDB01.ccgs_coreissue.dbo.BSegment_Primary BP WITH (NOLOCK) ON (MA.DestAccountNumber = BP.AccountNumber )
WHERE  T1.AccountNumber IN ('1100011127011249', '1100011139323376')
