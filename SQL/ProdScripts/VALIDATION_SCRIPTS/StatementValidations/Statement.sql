...
SET @LastSatementDate = '2025-06-30 23:59:57.000'
SET @Businessday = '2025-06-30 23:59:57.000'
SET @CCARD = 1
SET @EOD = 0
SET @Statement = 1
...
--WHERE SH.acctID = @BSacctId AND SH.StatementDate = @LastSatementDate
--ORDER BY SH.StatementDate DESC


--SELECT 'Summary====>',SH.acctId, SH.StatementID, SH.StatementDate, CreditPlanType, DisputesAmtNS, SHCC.CycleDueDTD, CurrentBalance, CurrentBalanceCO,Principal, PrincipalCO, OriginalPurchaseAmount, 
...
SELECT 'Statement====>', acctId,StmtDistControl,  BeginningBalance, Principal, CurrentBalance, CurrentBalanceCO,
AmountOfCreditsCTD, AmountOfCreditsRevCTD, AmountOfDebitsCTD, AmountOfDebitsRevCTD, AmountOfPurchasesCTD, RecoveryFeesCTD, CTDTotalInt, CTDTotalFees,
BeginningBalance+(AmountOfDebitsCTD-AmountOfDebitsRevCTD)-(AmountOfCreditsCTD-AmountOfCreditsRevCTD) CalculatedBalance, 
CurrentBalance -(BeginningBalance+(AmountOfDebitsCTD-AmountOfDebitsRevCTD)-(AmountOfCreditsCTD-AmountOfCreditsRevCTD)) DELTA
FROM StatementHeader SH WITH (NOLOCK)
WHERE SH.acctID = @BSacctId AND SH.StatementDate = @LastSatementDate
ORDER BY SH.StatementDate DESC
