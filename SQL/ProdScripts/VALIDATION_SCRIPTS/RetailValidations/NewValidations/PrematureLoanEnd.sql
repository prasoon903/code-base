...
AS
(
SELECT T.*, Activity, ILP.OriginalLoanEndDate, ILP.MaturityDate, ILP.PaidOffDate, ILP.ActualLoanEndDate, ROW_NUMBER() OVER(PARTITION BY ILP.PlanID ORDER BY ActivityOrder DESC) RN
FROM #TempData T
...
OrigEqualPmtAmt, EqualPaymentAmt, TCAPPaymentAmt, LoanEndDate, OriginalLoanEndDate, MaturityDate, PaidOffDate, ActualLoanEndDate
INTO #TempRecords
FROM CTE
WHERE RN = 1
AND OriginalLoanEndDate > ActualLoanEndDate

SELECT TOP 10 * FROM #TempRecords
