...
BEGIN
	...
	WHERE ILPS.PlanID = @CPSAcctID ORDER BY ILPS.ActivityOrder DESC
	...
	BEGIN 
		...
		BEGIN   
			...
			JOIN StatementHeader S WITH (NOLOCK) ON (S.acctId = SH.parent02AID AND SH.StatementID = S.StatementID AND SH.StatementDate = S.StatementDate)
			JOIN StatementHeaderEx SE WITH (NOLOCK) ON (S.acctId = SE.acctId AND S.StatementID = SE.StatementID AND S.StatementDate = SE.StatementDate)
			--JOIN @TempPlan TP ON (TP.planid = SCC.acctId)
			...
			INSERT INTO @TempJobID
				...
				JOIN @TempPlan TP ON (TP.planid = SCC.acctid)
				JOIN SummaryHeader SH WITH (NOLOCK) ON (SH.acctid = SCC.acctid AND SH.StatementID = SCC.StatementID) 
				JOIN CurrentSummaryHeader CSH WITH (NOLOCK) ON (SH.acctid = CSH.acctid AND SH.StatementID = CSH.StatementID AND SH.StatementDate = CSH.StatementDate) 
				--JOIN @TempPlan TP ON (TP.planid = SCC.acctid)
			...
			BEGIN
				INSERT INTO @tempCurrentunbilledterms  
					SELECT     
						...
						CASE WHEN CPCC.currentbalanceco > 0 THEN CPCC.AmountOfTotalDue ELSE CASE WHEN CPCC.AmtOfPayCurrDue > 0 THEN ILPR.RegularPaymentAmountForIteration ELSE 0 END END - (ISNULL(CP.AmountOfPaymentsCTD, 0) + ISNULL(TJI.CreditBalanceMovement, 0)) AS RegularPaymentAmountForIteration,  
				...
				BEGIN
					INSERT INTO @tempCurrentunbilledterms  
						SELECT 
							...
							0 AS LoanTerm ,   
							'840' AS CurrencyCode ,   
							ILPR.RegularPaymentAmountForIteration - (ISNULL(CP.AmountOfPaymentsCTD - CP.AmountOfreturnsCTD, 0) + ISNULL(TJI.CreditBalanceMovement, 0)) AS RegularPaymentAmountForIteration,  
							ISNULL(CP.AmountOfPaymentsCTD, 0) AS Paid, --ISNULL(CP.AmountOfPaymentsCTD - CP.AmountOfreturnsCTD, 0) AS Paid, 
							ILPR.DateOfTotalDue AS Due ,   
							@DateOfNextStmt,
							TJI.StmtHeader_UUID AS StatementUUID,
			...
			BEGIN
				INSERT INTO @TempILPSchedule
					SELECT
						...
						Paid,
						Due,
						CASE WHEN ISNULL(OriginalAmountDue, 0) > 0 THEN StatementUUID ELSE NULL END StatementUUID, --PLAT-51577
						OriginalAmountDue,
						CASE 
