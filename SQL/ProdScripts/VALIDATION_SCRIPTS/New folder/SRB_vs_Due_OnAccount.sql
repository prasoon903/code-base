...
SELECT  
BP.acctId BSAcctId, AmtOfPayCurrDue, AmtOfPayXDLate, AmountOfTotalDue, CycleDueDTD, SRBWithInstallmentDue, SBWithInstallmentDue, AccountGraceStatus, RunningMinimumDue, SystemStatus,
...
--AND BP.Acctid NOT IN (1409257, 13134832, 2354691, 2340232)
...
SELECT 
CPS.acctId CPSAcctId, BS.BSAcctID, CPCC.CycleDueDTD, CPCC.AmountOfTotalDue, CPCC.AmtOfPayCurrDue, CPCC.AmtOfPayXDLate, CPCC.AmountOfPayment30DLate, CPCC.SRBWithInstallMentDue, 
CPS.CreditPlanType, CAST(0 AS MONEY) DueAdjusted_PS, BS.IssueType
INTO #PSRecords
FROM .CPSgmentCreditCard CPCC WITH (NOLOCK) 
JOIN CPSgmentAccounts CPS WITH (NOLOCK) ON (CPCC.AcctID = CPS.acctId )
JOIN #BSRecords BS WITH (NOLOCK) ON (BS.BSAcctId = CPS.parent02AID)
--WHERE PIR.BusinessDay = '2021-11-29 23:59:57'
...
UPDATE #PSRecords SET AmountOfTotalDue = 0 WHERE IssueType = 'Issue1' AND AmountOfTotalDue > 0

UPDATE #BSRecords SET AmountOfTotalDue = 0 WHERE IssueType = 'Issue1' AND AmountOfTotalDue > 0
...
RunningMinimumDue = ' + TRY_CAST(AmountOfTotalDue AS VARCHAR) + ', RemainingMinimumDue = ' + TRY_CAST(AmountOfTotalDue AS VARCHAR) + 
', DaysDelinquent = 0, NoPayDaysDelinquent = 0, 
DAteOfOriginalPaymentDueDTD = ''2022-08-31 23:59:57'', DtOfLastDelinqCTD = NULL, FirstDueDate = ''2022-08-31 23:59:57''
WHERE acctID = ' + TRY_CAST(BSAcctID AS VARCHAR) BSegmentCreditCard
FROM #Accounts_Fixed
WHERE CycleDueDTD > 1
ORDER BY BSAcctID
...
(
	...
	WHERE C.BusinessDay > '2022-08-31 23:59:57'
	AND DENAME = 200
)
SELECT *,
CASE
...
FROM CTE C
WHERE Ranking = 1
AND OldValue = '0' AND NewValue = '1' AND IssueType = 'Issue2'
