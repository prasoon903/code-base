...
DECLARE @TranCode VARCHAR(20) = '0313I'
...
VALUES (''' + @JIRAID + ''', ' + TRY_CONVERT(VARCHAR, acctID) + ', '''+RTRIM(AccountNumber)+''', ' + TRY_CONVERT(VARCHAR, TransactionAmount) + ', ''03''' + ', '''+ @TranCode +'''' + ', ''' + TRY_CONVERT(VARCHAR(20), TranTime, 20) + ''')'
FROM #TempCombined
ORDER BY TranTime
