...
into #cp_Auth
from CoreIssue_Snapshot_Snap_221001.dbo.bsegment_primary bp with(nolock) join
CoreIssue_Snapshot_Snap_221001.dbo.ccard_primary cp with(nolock) on cp.accountnumber = bp.accountnumber join
CoreIssue_Snapshot_Snap_221001.dbo.auth_primary ap with(nolock) on ap.accountnumber = cp.accountnumber and ap.TransactionLifeCycleUniqueID = cp.TransactionLifeCycleUniqueID
...
into #cp1
from CoreIssue_Snapshot_Snap_221001.dbo.bsegment_primary bp with(nolock) join
CoreIssue_Snapshot_Snap_221001.dbo.ccard_primary cp with(nolock) on cp.accountnumber = bp.accountnumber 
