CREATE OR ALTER   PROCEDURE USP_GetPaymentTerms
	@AccountID				INT,   
	@PlanID					INT,   
	...
	BEGIN 
		...
		BEGIN   
			...
			SELECT 
				...
				, CASE WHEN @MergeIndicator = 1 THEN RANK() OVER (PARTITION BY SCC.acctId ORDER BY SH.StatementDate ASC) - 1 ELSE 1 END
			...
			*/
			...
			INSERT INTO #TempBilledTerms   
				SELECT 
					...
					CASE WHEN (ISNULL(TS.WaiveMinDue, 0) = 1 AND @LastTermDate > @OriginalLoanEndDate 
					AND (@PaidOffDate IS NULL OR @PaidOffDate > ILP.LastStatementDate)) OR (TJI.CurrentDue <= 0 AND ISNULL(TJI.CreditBalanceMovement, 0) < 0 AND ISNULL(TJI.CSHCurrentBalance, 0) < 0) OR ISNULL(ILP.IsDeferCycle,0) = 1 THEN 1 ELSE 0 END AS SkipSchedule,
					0 AS AmountPaid,
					CASE	WHEN ILP.DateOfTotalDue = CASE WHEN @LastTermDate > @OriginalLoanEndDate THEN @LastTermDate ELSE @OriginalLoanEndDate END THEN @OriginalLastMonthPayment
								WHEN ILP.DateOfTotalDue < CASE WHEN @LastTermDate > @OriginalLoanEndDate THEN @LastTermDate ELSE @OriginalLoanEndDate END THEN @OriginalMonthlyPayment
								ELSE 0 
			...
			BEGIN
				INSERT INTO #tempCurrentunbilledterms  
					SELECT     
						...
						0 AS AmountAppliedTowardsMonth,
						CASE 
							WHEN	(((@dateofTotalDue <> ILPR.DateOfTotalDue AND @PaidOffDate IS NULL) OR (TJI.CurrentDue <= 0 AND ISNULL(TJI.CreditBalanceMovement, 0) < 0 AND ISNULL(TJI.CSHCurrentBalance, 0) < 0)
			...
			--SELECT CC.OriginalPurchaseAmount - (CC.currentbalanceco + CPS.CurrentBalance  ) - TU.UnBilledAmount, TU.UnBilledAmount, CC.OriginalPurchaseAmount, CC.currentbalanceco, CPS.CurrentBalance
			--FROM #TempPlan TT
			...
			SELECT @CountArray  = COUNT(1)  FROM #TempBilledTerms WHERE SkipSchedule  = 0 AND Ranking = 1
			SELECT @CountArray  = @CountArray + COUNT(1)  FROM #tempCurrentunbilledterms WHERE SkipSchedule  = 0 
			...
			IF(@OriginalLoanEndDate > @LoanEndDate)
				...
				BEGIN
					...
					OriginalDue,
					ActualStatementdate 
					FROM #tempTermsOnPaymentAppliedExtended
				END
