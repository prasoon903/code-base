...
SELECT @RecordCount = COUNT(1) FROM TEMP_IncorrectCPMCorrection WITH (NOLOCK) WHERE JobStatus = 0
		...
		BEGIN TRY
			...
			BEGIN TRY
				BEGIN TRANSACTION
					...
					FROM CCard_Primary CP
					JOIN #TEMP_IncorrectCPMCorrection_JobTable T ON (CP.AccountNumber = T.AccountNumber AND CP.TranId = T.TranID)
