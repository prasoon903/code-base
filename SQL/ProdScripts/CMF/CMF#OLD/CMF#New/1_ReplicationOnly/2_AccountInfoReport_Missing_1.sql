--script need to  run  ccgs_coreissue primary  database of platproduction
...
-- 29 rows update 
      ...
      SET    AIR.DaysDelinquent = SE.DaysDelinquent,
			 ...
			 AIR.LAstStatementdate				= '2022-06-30 23:59:57.000',
			 AIR.DateOfNextStmt					= '2022-07-31 23:59:57.000'
      FROM   AccountInfoForReport AIR
             JOIN StatementHeader SH WITH(NOLOCK)
               ON ( AIR.Businessday = '2022-06-30 23:59:57.000'
                    AND AIR.BSAcctid = SH.acctId)
			JOIN StatementHeaderEx SE WITH(NOLOCK)
			  ON (SH.acctId = SE.acctId
                    AND SH.StatementID = SE.StatementID)
					where   SH.acctid in (6730375) 
					 		AND SH.StatementDate = '2022-06-30 23:59:57.000'
