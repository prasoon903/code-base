DECLARE @JiraID VARCHAR(20) = 'COOKIE-292364'
...
FROM dbo.TransactionCreationTempData WITH (NOLOCK)
WHERE JiraID = @JiraID
GROUP BY JobStatus

INSERT INTO #TempTxns
SELECT TranID, TransactionStatus
FROM dbo.CreateNewSingleTransactionData WITH (NOLOCK)
WHERE Reason = @JiraID
...
FROM dbo.Trans_In_Acct WITH (NOLOCK)
...
FROM dbo.CreateNewSingleTransactionData C WITH (NOLOCK)
JOIN BSegment_Primary BP WITH (NOLOCK) ON (BP.accountNumber = C.AccountNumber)
JOIN dbo.MergeAccountJob M WITH (NOLOCK) ON (M.SrcAccountNumber = BP.accountNumber)
WHERE TransactionStatus = 1 AND TranID IS NULL
ORDER BY C.TranTime
...
SELECT BSAcctId,PostingRef,hostmachinename, AccountNumber, * FROM dbo.CCard_Primary CP WITH (NOLOCK) WHERE TranId IN ( 81302157672)
--81302157672
--81302037604

SELECT PostingRef, EntryReason, * FROM dbo.CreateNewSingleTransactionData CP WITH (NOLOCK) WHERE TranId IN ( 57671807206, 57671807207)
...
SELECT * FROM dbo.ErrorTNP WITH (NOLOCK)

SELECT * FROM CCArd_Primary WITH (NOLOCK) WHERE TranID = 55971261461

SELECT * FROM CCArd_Primary WITH (NOLOCK) WHERE TranID IN (SELECT TranID FROM dbo.ErrorTNP WITH (NOLOCK))
...
SELECT * FROM dbo.CCardLookup WITH (NOLOCK) WHERE Lutid ='órganizationname '
...
FROM dbo.CreateNewSingleTransactionData CD WITH (NOLOCK)
JOIN BSegment_Primary BP WITH (NOLOCK) ON (CD.AccountNumber = BP.AccountNumber)
WHERE TransactionStatus = 1 AND TranID IS NULL

SELECT *
FROM dbo.EmbossingAccounts E WITH (NOLOCK)
JOIN dbo.BSegment_Primary BP WITH (NOLOCK) ON (E.Parent01AID = BP.acctId AND E.ECardType = 0)
JOIN dbo.CreateNewSingleTransactionData ST WITH (NOLOCK) ON (ST.AccountNumber = BP.AccountNumber AND ST.TransactionStatus = 1 AND BP.InstitutionID = 6981)
...
FROM dbo.CreateNewSingleTransactionData ST WITH (NOLOCK)
LEFT JOIN dbo.BSegment_Primary BP WITH (NOLOCK) ON (ST.AccountNumber = BP.AccountNumber AND ST.TransactionStatus = 1 AND BP.InstitutionID = 6981)
LEFT JOIN dbo.EmbossingAccounts E WITH (NOLOCK) ON (E.Parent01AID = BP.acctId)
...
FROM dbo.CreateNewSingleTransactionData ST WITH (NOLOCK)
LEFT JOIN dbo.BSegment_Primary BP WITH (NOLOCK) ON (ST.AccountNumber = BP.AccountNumber AND ST.TransactionStatus = -2 AND BP.InstitutionID = 6981)
...
SELECT TOP(10) 'MergeAccountJob====>', SrcRemainingMinDue, DESTRemainingMinDue, * FROM dbo.MergeAccountJob WITH (NOLOCK) ORDER BY Skey DESC
...
FROM dbo.CreateNewSingleTransactionData ST WITH (NOLOCK)
LEFT JOIN dbo.MergeAccountJob MA WITH (NOLOCK) ON (ST.AccountNumber = MA.SRCAccountNumber)
LEFT JOIN dbo.BSegment_Primary BP WITH (NOLOCK) ON (MA.DestAccountNumber = BP.AccountNumber )
WHERE ST.TransactionStatus = -2
