...
--SELECT * FROM #TempAllRecords WHERE AccountUUID = '89be6a38-20f7-4352-80c3-5cad3a043474'
...
FROM #TempRecords
--WHERE acctID = 12714731
GROUP BY acctID
)
SELECT ROW_NUMBER() OVER(PARTITION BY C.acctID ORDER BY StatementDate) RN, C.*, StatementDate, COALESCE(LastStatementDate, DATEADD(SS, 86397, TRY_CONVERT(DATETIME, EOMONTH(DATEADD(MM, 0, ClearingLastStatement))))) LastStatementDate, 
...
--SELECT * FROM #StatementData WHERE acctID = 12714731
--SELECT * FROM #TempRecords WHERE acctID = 12714731
--SELECT * FROM #TempInt1
--2405264
...
BEGIN
	...
	--select * FROM #TempInterest T1  where t1.acctid = 2702226
...
--SELECT * FROM #StatementData WHERE acctID = 384538
--SELECT * FROM #TempInterest WHERE acctID = 200783 ORDeR BY StatementDate
...
S.AmountOfPaymentsCTD TotalAmountToApply,
...
BEGIN
	...
	--Select 'BEFORE PROCESSING',* from #TempJobToProcess 
	...
	CASE	WHEN NextDQ >= 1 
			THEN 
				CASE	WHEN T2.RN = 2 AND T2.SRB_Calc > 0 AND (T2.MiniDue_Calc+T2.AmountOfPayment210DLate+T2.AmountOfPayment180DLate+T2.AmountOfPayment150DLate+T2.AmountOfPayment120DLate+T2.AmountOfPayment90DLate+T2.AmountOfPayment60DLate+T2.AmountOfPayment30DLate+T2.AmtOfPayXDLate) >= T2.SRB_Calc 
						...
						THEN 
							CASE	WHEN (T2.SRB_Calc - (T1.AmountOfPayment210DLate+T1.AmountOfPayment180DLate+T1.AmountOfPayment150DLate+T1.AmountOfPayment120DLate+T1.AmountOfPayment90DLate+T1.AmountOfPayment60DLate+T1.AmountOfPayment30DLate+T1.AmtOfPayXDLate+T1.AmtOfPayCurrDue)) > 0 AND (T2.SRB_Calc - (T1.AmountOfPayment210DLate+T1.AmountOfPayment180DLate+T1.AmountOfPayment150DLate+T1.AmountOfPayment120DLate+T1.AmountOfPayment90DLate+T1.AmountOfPayment60DLate+T1.AmountOfPayment30DLate+T1.AmtOfPayXDLate+T1.AmtOfPayCurrDue)) <= T2.AmtOfPayCurrDue
									THEN (T2.SRB_Calc - (T1.AmountOfPayment210DLate+T1.AmountOfPayment180DLate+T1.AmountOfPayment150DLate+T1.AmountOfPayment120DLate+T1.AmountOfPayment90DLate+T1.AmountOfPayment60DLate+T1.AmountOfPayment30DLate+T1.AmtOfPayXDLate+T1.AmtOfPayCurrDue)) 
									WHEN (T2.SRB_Calc - (T1.AmountOfPayment210DLate+T1.AmountOfPayment180DLate+T1.AmountOfPayment150DLate+T1.AmountOfPayment120DLate+T1.AmountOfPayment90DLate+T1.AmountOfPayment60DLate+T1.AmountOfPayment30DLate+T1.AmtOfPayXDLate+T1.AmtOfPayCurrDue)) > 0 AND (T2.SRB_Calc - (T1.AmountOfPayment210DLate+T1.AmountOfPayment180DLate+T1.AmountOfPayment150DLate+T1.AmountOfPayment120DLate+T1.AmountOfPayment90DLate+T1.AmountOfPayment60DLate+T1.AmountOfPayment30DLate+T1.AmtOfPayXDLate+T1.AmtOfPayCurrDue)) > T2.AmtOfPayCurrDue
									THEN T2.AmtOfPayCurrDue
									ELSE 0 
							END
...
CASE WHEN T1.StatementDate BETWEEN T1.ClearingLastStatement AND T1.PCNextStatement AND T1.CycleDueDTD >= 3 AND T4.CycleDueDTD <= 2 AND T1.StatementDate <= MaxPCDate THEN 'YES' ELSE 'NO' END PHPImpact, 
I.acctID, I.AccountNumber, I.AccountUUID, I.MinClearingDate, I.MaxPCDate, T1.ClearingLastStatement, T1.PCNextStatement, 
CASE WHEN T1.StatementDate BETWEEN T1.ClearingLastStatement AND T1.PCNextStatement THEN T1.TransactionAmount ELSE NULL END DisputeAmountOfCycle, 
CASE WHEN T1.StatementDate BETWEEN T1.ClearingLastStatement AND T1.PCNextStatement THEN T2.APR ELSE NULL END APR, 
T2.InterestAtCycle, 
...
--WHERE T1.RN > 1
--WHERE T1.acctID = 21929358
ORDER BY T1.acctID, T1.RN



--SELECT * FROM #ProcessedData WHERE acctID = 384538 ORDER BY acctID, RN
...
select * from #Final_AllAccounts where acctid = 21929358
...
SELECT * FROM #Final_AllAccounts WHERE acctID = 60770 ORDER BY acctID, RecordNumber

select * from #TempAllRecords where acctid = 4719650
4125291
...
SELECT * FROM #TempRecords WHERE AccountUUID = 'e25cde34-3384-40b4-aaf5-fbb73619953e'

SELECT * FROM #TempInterest WHERE acctID = 4719650
SELECT * FROM #TempDataProcessed WHERE acctID = 4719650
SELECT * FROM #StatementData WHERE acctID = 200783
...
SELECT COUNT(distinct acctid)  FROM #FinalImpacted_All WHERE PHPImpact = 'YES' AND AmountOfPaymentsCTD > 0 --101

SELECT COUNT(distinct acctid)  FROM #FinalImpacted_All WHERE PHPImpact = 'YES' AND SRB_CALC <= 0 and OriginalDQ > 2 --189

SELECT COUNT(distinct acctid)  FROM #FinalImpacted_All WHERE PHPImpact = 'YES' AND SRB_CALC > 0 and 
AmountAppliedDueToSRB > 0 and currentbalance <> amountoftotaldue_original --159
...
SELECT * FROM #FinalImpacted_All 
SELECT * FROM #FinalImpacted_All WHERE PHPImpact = 'YES' AND AmountOfPaymentsCTD > 0 --116
SELECT *  FROM #FinalImpacted_All WHERE PHPImpact = 'YES' AND SRB_CALC <= 0 and OriginalDQ > 2 --908
SELECT amountoftotaldue_original,currentbalance,SRB_CALC,AmountAppliedDueToSRB,*  FROM #FinalImpacted_All WHERE 
PHPImpact =
'YES'AND 
SRB_CALC > 0 and AmountAppliedDueToSRB > 0 and AmountOfTotalDue_Calc < 25 and
currentbalance <> amountoftotaldue_original and srb_original < currentbalance--236


select * from #FinalImpacted_All where acctid = 21929358--21581352


SELECT *  FROM #FinalImpacted_All where srb_original < currentbalance and AmountAppliedDueToSRB > 0
0.00
...
SELECT distinct acctid  FROM #FinalImpacted_All WHERE PHPImpact = 'YES' and srb_calc <=0--189
SELECT distinct acctid  FROM #FinalImpacted_All WHERE PHPImpact = 'YES' and srb_calc > 0 and amountofpaymentsctd >0--82
SELECT distinct acctid  FROM #FinalImpacted_All WHERE PHPImpact = 'YES' and srb_calc > 0 and srb_calc < AmountOfTotalDue_Calc--455
 
 SELECT srb_calc,amountofpaymentsctd,AmountOfTotalDue_Calc,* FROM #FinalImpacted_All WHERE PHPImpact = 'YES'

SELECT count( distinct acctid)  FROM #FinalImpacted_All WHERE PHPImpact = 'YES' and manualstatus not in (15996,16000)
and 
...
T1.RN StatementOrder, T1.StatementDate, T1.LastStatementDate ActualImpactedStatement, T1.CurrentBalance, T1.AmountOfPaymentsCTD, T1.CycleDueDTD OriginalDQ, T2.CycleDueDTD CalculatedDQ,
...
select * into  From #Final_AllAccounts where acctid in (21929358)

21929358,1718413,1366254,1382259,384538,2702226
...
--AND CurrentBalance_calc > 0 AND AmountOfTotalDue_Proj <= 0
...
SELECT * FROM #Final_AllAccounts WHERE 
--acctID = 200783 
acctid = 400624
ORDER BY acctID, RecordNumber
...
AND SystemStatus <> 14
--AND ManualStatus NOT IN (15996, 16000, 16326, 16330)
AND (interestatcycle - AmountOfInterestCTD) <= 0
...
--SELECT COUNT(DISTINCT ACCOUNTUUID) FROM ##TempRawData --4404


--SELECT COUNT(DISTINCT acctId) FROM #Final_AllAccounts --3882 POD1
