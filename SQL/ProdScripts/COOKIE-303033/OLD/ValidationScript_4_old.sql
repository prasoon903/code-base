...
INTO #Final_AllAccounts
FROM #TempData T1
JOIN #TempCalculatedData T2 ON (T1.acctID = T2.acctID AND T1.RN = T2.RN)
JOIN #TempCalculatedData T4 ON (T2.acctId = T4.acctID AND T4.RN = T2.RN+1)
LEFT JOIN #TempData T3 ON (T1.acctID = T3.acctID AND T1.RN = T3.RN+1)
JOIN #AccountData I WITH (NOLOCK) ON (I.acctID = T2.acctID)
--WHERE T1.RN > 1
--WHERE T1.acctID = 21929358
