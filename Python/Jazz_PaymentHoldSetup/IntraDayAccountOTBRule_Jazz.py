...
Description:  Jazz | 18355 | Implement account-level payment hold logic 
=============================================
'''

from IntraDayAccountOTBRule_Jazz_setup import SetUp
import pyodbc
import logging
import os
import sys
...
def ImportFileToDB(InputDir, FileName):
    ...
    Query = "TRUNCATE TABLE " + CI_DB + "..IntraDayAccountOTBRule_External_Jazz"
    ...
    BCP = "bcp " + CI_DB + "..IntraDayAccountOTBRule_External_Jazz IN " + File + " -b 5000 -h TABLOCK " \
                "-f " + dir_path + "IntraDayAccountOTBRule_External_JAZZ.xml -e " + str(LogDir) + "Error1_in.log -o "  \
...
def DataValidation(FileID, FileName):
    ...
    Query = "EXEC " + CI_DB + "..USP_InsertInto_IntraDayAccountOTBRule_Jazz '" + FileID + "', '" + FileName + "'" 
    ...
    if RowCount > 0:
        ...
        if FileError == 0:
            ErrorMessage = 'No Error'
            MessageLogger.info("USP_InsertInto_IntraDayAccountOTBRule_Jazz executed successfully")
            Error = False
        elif FileError == 1:
...
def EmailStructure(Type, EmailBody):
    # Type: "Head" and "Tail"
    if Type == "Head":
        # EMailSubject = "ALERT | Jazz ::: Milk Replay Schedules file processing"
...
if __name__ == '__main__':
    ...
    if Error is False:
        ...
        EMailSubject = "Alert | Jazz - " + Environment + " :: Account Level Payment Hold File Processing"
