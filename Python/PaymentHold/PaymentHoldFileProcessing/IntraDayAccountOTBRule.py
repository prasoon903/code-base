...
ExceptionDir = Config['PaymentHoldSetup']['POTBExceptionFile']
POTBExceptionPrefix = Config['PaymentHoldSetup']['POTBExceptionPrefix']
POTBExceptionSuffix = Config['PaymentHoldSetup']['POTBExceptionSuffix']

POTBAWSEnvironment=int(Config['DEFAULT']['AWSEnvironment'])
POTBAWS_secret_name=Config['DEFAULT']['ses_smtp_secret_name']
POTBAWS_region_name=Config['DEFAULT']['ses_region_name']
POTBAWS_service_name=Config['DEFAULT']['ses_service_name']
POTBSES_smtp_url=Config['DEFAULT']['ses_smtp_url']
POTBAWSPort=Config['DEFAULT']['ses_smtp_port']
...
def DirectoryValidation(InputDir, OutputDir, ErrorDir, LogDir, ExceptionDir):
    ErrorFlag = True
    ErrorNumber = 0
    Message=""
    Check = True
    if InputDir == "":
        ...
        ErrorFlag = True
        Message = "Environment variable for ExceptionDir folder is not set"
    else:
        ErrorFlag = False
        Message = "NO ERROR FOR FOLDER SET UP"
    ...
    try:
        ...
        print("Error in directory validation")

    return InputDir, OutputDir, ErrorDir, LogDir, ExceptionDir, ErrorFlag    
###################################################################################################
...
def get_secret(): 
    ...
    secret_name = POTBAWS_secret_name
    region_name = POTBAWS_region_name 
    ...
    try: 
        ...
        client = session.client(
            service_name=POTBAWS_service_name,
...
def FileTotalRecordCount(FileName):
    MessageLogger.info("Count the total number of records in the file")
    IsError = False
    Message = ""
    RecordCount = 0

    def _count_generator(reader):
        bufferReader = reader(1024 * 1024)
        while bufferReader:
            yield bufferReader
            bufferReader = reader(1024 * 1024)
    ...
    try:
        with open(FileName, 'rb') as fp:
            ...
            RecordCount = sum(buffer.count(b'\n') for buffer in c_generator)
            MessageLogger.info('Total records in file:' + str(RecordCount))
    except:
        IsError = True
        Message = 'Error in reading the total records in file'
        MessageLogger.debug(Message)

    return IsError, Message, RecordCount

def FileToProcess(FileName, RecordCountInFile):
    FileStatus = ''
    ErrorFlag = 0
    ...
    Query = "EXEC " + CI_DB + "..USP_GetStatusOfPaymentHoldFileProcessing '" + FileName + "', " + str(RecordCountInFile)
...
def FileDetails(FileName, MessageLogger):
...
def DataValidation(FileID, FileName, ExceptionFileName, RecordCountInFile):
    ...
    Query = "EXEC " + CI_DB + "..USP_InsertInto_IntraDayAccountOTBRule_Internal '" + FileID + "', '" + FileName + "' ,'" + str(InstitutionID) + "', '" + POTBPolicy  +  "', '" + str(POTBIdentifier) + "'" +  ", '" + str(ExceptionFileName) + "', " + str(ShardMigrationStatusCheck) + ", " + str(RecordCountInFile)
    ...
    try:
        ...
        Error = True
        MessageLogger.error("ERROR IN EXECUTING USP_InsertInto_IntraDayAccountOTBRule_Internal :: " + str(e))

    if RowCount > 0 and Error == False:
        for r in Row:
        ...
        if FileError == 0:
            ...
            ErrorMessage = 'Total records in file is not equal to records in DB'
            #MessageLogger.info('ErrorFlag: ' + str(FileError) + '::' + ErrorMessage)
        elif FileError == 4:
        ...
        if FileError == 0:
            if SuccessCount > 0:
                Query = "EXEC " + CI_DB + "..USP_Archiveintradayaccountotbrule_Internal '" + FileName + "'"
...
def SendMail(EmailSubject, EmailBody):
    try:
        if POTBAWSEnvironment == 0:
            ...
            server = smtplib.SMTP(POTBSES_smtp_url, POTBAWSPort)
...
def GenerateExceptionFile(FileID, FileName, ExceptionFile, MessageLogger ):
    ...
    ExceptionFileName = ExceptionDir + "\\" + ExceptionFile

    if POTBPolicy == 'G2':
        Query = "SELECT RTRIM(AccountNumber) AcctNumber, InstitutionID, DecisionFileTimeStamp, NontrivialPaymentSizeThreshold1, NontrivialPaymentSizeThreshold2, BusinessDaysHeldifPaymentRiskLow, " \
                "BusinessDaysHeldifPaymentRiskMedium, BusinessDaysHeldifPaymentRiskHigh, BusinessDaysHeldifPaymentRiskVeryHigh, IntradayPaymentThreshold1, IntradayPaymentThreshold2, " \
                "IntradayPaymentThreshold3, IntradayPaymentThreshold4, IntradayAttemptedSpendThreshold1, IntradayAttemptedSpendThreshold2, RIPThreshold1, RiskScore, ACHBankAccountThreshold, " \
                "VIPFlag, TestFlag, PolicyVersion, PolicySubVersion, ModelScoreThreshold1, ModelScoreThreshold2, ModelScoreThreshold3, ModelScoreThreshold4, ModelScoreThreshold5 " \
                "FROM " + CI_DB + "..POTB_ExceptionRecord WITH (NOLOCK) " \
                "WHERE FileName = '" + FileName + "' " \
                "AND FileID = '" + FileID + "' " 
    else:
        Query = "SELECT UniversalUniqueID, InstitutionID, IntradayThreshholdAmount, HeldDays_LessThanIntradayThreshhold, HeldDays_GreaterThanIntradayThreshhold, StartDate, Transactioncode, ACHSucessfulPaymentThreshold " \
                "FROM " + CI_DB + "..POTB_ExceptionRecord WITH (NOLOCK) " \
                "WHERE FileName = '" + FileName + "' " \
                "AND FileID = '" + FileID + "' " 
    try:
        ...
        if ConnectionStatus == True:
            ...
            if TotalRecords > 0:
                MessageLogger.debug("ExceptionFileName: " + ExceptionFileName)
                ResultDataFrame.to_csv(ExceptionFileName, sep=',', index=False, mode='w', escapechar='\n', quotechar='"'  )
...
def GetExceptionFileName(FileName, MessageLogger):
    ExceptionFile = ""

    try:
        FileName_WithoutExt = os.path.splitext(FileName)[0]
        ExceptionFile = POTBExceptionPrefix + FileName_WithoutExt + POTBExceptionSuffix + POD +".csv"
...
def GetShardMigrationStatus(MessageLogger):
    MessageLogger.info("INSIDE GetShardMigrationStatus ")
    Sharding = 0
    RowCount = 0
    Query = "SELECT ISNULL(Sharding, 0) Sharding FROM "+ CI_DB + "..Org_Balances WITH(NOLOCK) WHERE AcctID = " + str(InstitutionID)
    ...
    try:
        ...
        MessageLogger.info("EXITING GetShardMigrationStatus BLOCK")

    except Exception as e:
        MessageLogger.error("ERROR IN GetShardMigrationStatus BLOCK " + str(e))
...
if __name__ == '__main__':
    ...
    InputDir, OutputDir, ErrorDir, LogDir, ExceptionDir, Error = DirectoryValidation(InputDir, OutputDir, ErrorDir, LogDir, ExceptionDir)
    ...
    if Error is False:
        ...
        try: 
            ...
            Message = "****************** PAYMENT HOLD FILE PROCESSING STARTS ******************"
            MessageLogger.info(Message)
            Message = "Start Time::: " + str(LogDateTime)
            MessageLogger.info(Message)
            MessageLogger.info("InputDir: " + InputDir)
            MessageLogger.info("OutputDir: " + OutputDir)
            MessageLogger.info("ErrorDir: " + ErrorDir)
            MessageLogger.info("LogDir: " + LogDir)
            MessageLogger.info("ExceptionDir: " + ExceptionDir)

            ShardMigrationStatusCheck = GetShardMigrationStatus(MessageLogger)
            ...
            for File in glob.glob("*"):
                ...
                if ABORT is False:
                    ABORT, ErrorMessage, RecordCountInFile = FileTotalRecordCount(FileName)
                    if ABORT:
                        shutil.move(InputDir + FileName, ErrorDir + FileName)
                        ErrorReason = ErrorMessage

                if ABORT is False:
                    ABORT, ErrorMessage = FileToProcess(FileName, RecordCountInFile)
                    if ABORT:
                        shutil.move(InputDir + FileName, ErrorDir + FileName)
                        ErrorReason = ErrorMessage
                ...
                if ABORT is False:
                    ExceptionFileName = GetExceptionFileName(FileName, MessageLogger)
                    ABORT, ErrorFlag, ErrorMessage, TotalCount, SuccessCount, ErrorCountForAccountFile, ArchiveCount, notprocessedCount, FileDetailsEmailBody = DataValidation(FileNameId, FileName, ExceptionFileName, RecordCountInFile)
                ...
                if ABORT is False and ShardMigrationStatusCheck == 1:
                    ExceptionEmailData = ExceptionEmailData + GenerateExceptionFile(FileNameId, FileName, ExceptionFileName, MessageLogger)
            ...
            EmailBody = EmailBody + ExceptionEmailData
