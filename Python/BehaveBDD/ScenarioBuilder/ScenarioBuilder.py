...
ProcessedFileDir = S1.ProcessedFile
...
def DirectoryValidation(InputDir, OutputDir, ErrorDir, LogDir, ResponseFileDir, ProcessedFileDir, DumpedFileDir):
    ErrorFlag = True
    ErrorNumber = 0
    Message = ""
    if InputDir == "":
    ...
    elif ProcessedFileDir == "":
        ErrorFlag = True
        Message = "Environment variable for ProcessedFile folder is not set"
    elif DumpedFileDir == "":
        ErrorFlag = True
    ...
    if ErrorFlag == False:
        ...
        ProcessedFileDir = ProcessedFileDir + "\\"
        DumpedFileDir = DumpedFileDir + "\\"

    return InputDir, OutputDir, ErrorDir, LogDir, ResponseFileDir, ProcessedFileDir, DumpedFileDir, ErrorFlag
...
def DumpingToDB(File):
    MessageLogger.info("INSIDE DumpingToDB")

    FileName = ResponseFileDir + File
    try:
        #print(FileName)
        CMD = "sqlcmd /S " + str(S1.SERVERNAME_POD1) + " /d " + str(S1.CI_DB_POD1) + " -E -i " + FileName
        MessageLogger.info("CMD: " + CMD)
        subprocess.run(CMD)
        # shutil.move(FileName, DumpedFileDir + File)
...
def ReadingTheFileDF(FileName):
    ...
    # MessageLogger.info(DF)

    MessageLogger.info("DATA CONVERTED FROM XLS TO DF")

    # DF1 = pd.DataFrame()
    # MessageLogger.info(DF)
    DF_PostTime_Sorted = DF.sort_values(by='PostTime')
    DF_PostTime_Sorted[['TranCode', 'Description']] = DF_PostTime_Sorted['TransactionDescription'].str.split(' = ', expand=True)
    MessageLogger.info(DF_PostTime_Sorted)
    DF_PostTime_Sorted.to_excel(ProcessedFileDir+FileName, index=False)
    # MessageLogger.info(DF_PostTime_Sorted)
    LoopCount = 0
    # print(DF_PostTime_Sorted)
...
if __name__ == '__main__':
    ...
    InputDir, OutputDir, ErrorDir, LogDir, ResponseFileDir, ProcessedFileDir, DumpedFileDir, Error = (
        DirectoryValidation(InputDir, OutputDir, ErrorDir, LogDir, ResponseFileDir, ProcessedFileDir, DumpedFileDir))
