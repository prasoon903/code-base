...
def fn_ScenarioExecute():
    ...
    def fn_PreSQLFile():
        FolderPath = os.path.join(c.BasePath, 'PreSQLScripts\\CoreIssue' )
        ...
        for File in glob.glob("*.sql"):
            ...
            FileName = File
            try:
                CMD = "sqlcmd /S " + str(Configuration["DBServer"]) + " /d " + str(Configuration["YourDBNames"]["COREISSUE"]) + " -E -i " + FileName
