...
def step_impl(context,APIName):
    ...
    assert len(context.custom_table[0]) ==2 ,fn_InsertFeatureStepExecutionInfo(EXECUTION_ID,context.feature.name,context.scenario.name,"Save tag into variable",'Fail More than two column')
    try:
            ...
            if len(res) > 0:
                ...
                if(len(res1) >0 ):
                    ...
                    if(loopcount> 10):
                        ...
                        print(len(res2))
                        if(len(res2) > 0 ):
                            print("Transaction posting is in ErrorTNP", context.TransactionID)
                            bflag = True


        assert len(res2) <= 1, fn_InsertFeatureStepExecutionInfo(EXECUTION_ID, context.feature.name,
                                                                context.scenario.name, "Verify Transaction in DB",
...
def step_impl(context):
    try:
        print("Code with new execuationid", get_global_EXECUTION_ID())
        EXECUTION_ID = get_global_EXECUTION_ID()
        fn_InsertFeatureStepExecutionInfo(EXECUTION_ID,context.feature.name,context.scenario.name,"Age TestAccount",'InProgress')
        payLoad = fn_CreateJsonPayloadUsingTable(EXECUTION_ID,context.table)
        response = fn_HitAPI('AgingAPI',payLoad)
        context.response = response
        context.AccountNumber = payLoad['AccountNumber']
        #context.Aging = payLoad['TransactionAmount']
        fn_InsertFeatureStepExecutionInfo(EXECUTION_ID,context.feature.name,context.scenario.name,"Age TestAccount",'Done')
    except AssertionError as ae:
        # Handle AssertionError specifically (if needed)
        print(f"AssertionError occurred: {ae}")
        raise Exception('An error occurred during this step')
    except Exception as e:
        fn_InsertFeatureStepExecutionInfo(EXECUTION_ID,context.feature.name,context.scenario.name,"Aging of account",'Fail ')
...
def step_impl(context):
    try:
        print("Code with new execuationid", get_global_EXECUTION_ID())
        EXECUTION_ID = get_global_EXECUTION_ID()
        fn_InsertFeatureStepExecutionInfo(EXECUTION_ID,context.feature.name,context.scenario.name,"Initiate Dispute",'InProgress')
        payLoad = fn_CreateJsonPayloadUsingTable(EXECUTION_ID,context.table)
        response = fn_HitAPI('AgingAPI',payLoad)
        context.response = response
        context.AccountNumber = payLoad['AccountNumber']
        #context.Aging = payLoad['TransactionAmount']
        fn_InsertFeatureStepExecutionInfo(EXECUTION_ID,context.feature.name,context.scenario.name,"Initiate Dispute",'Done')
    except AssertionError as ae:
        # Handle AssertionError specifically (if needed)
        print(f"AssertionError occurred: {ae}")
        raise Exception('An error occurred during this step')
    except Exception as e:
        fn_InsertFeatureStepExecutionInfo(EXECUTION_ID,context.feature.name,context.scenario.name,"Initiate Dispute",'Fail ')
...
def step_impl(context):
    try:
        ...
        EXECUTION_ID = get_global_EXECUTION_ID()
        fn_InsertFeatureStepExecutionInfo(EXECUTION_ID, context.feature.name, context.scenario.name,
                                          "Get StatementMetaData", 'InProgress')
        payLoad = fn_CreateJsonPayloadUsingTable(EXECUTION_ID, context.table)
...
def step_impl(context):
    try:
        ...
        EXECUTION_ID = get_global_EXECUTION_ID()
        fn_InsertFeatureStepExecutionInfo(EXECUTION_ID, context.feature.name, context.scenario.name,
                                          "Get PlanSegmentDetails", 'InProgress')
        payLoad = fn_CreateJsonPayloadUsingTable(EXECUTION_ID, context.table)
...
def step_impl(context):
    ...
    os.startfile(r"MoveTrace.bat")
    time.sleep(30)
    Output = fn_VerifyTracefiles('sched->Exec() starting', Configuration["CITraceFileLocation"],'wfTnpNad','CITNP')
    os.chdir(cwd)
    assert Output!=False ,fn_InsertFeatureStepExecutionInfo(EXECUTION_ID,context.feature.name,context.scenario.name,f'Start CITNP','CITNP  Fatal - Assert')
...
def step_impl(context):
    ...
    os.startfile(r"MoveTrace.bat")
    time.sleep(30)
    Output = fn_VerifyTracefiles('sched->Exec() starting',Configuration["CITraceFileLocation"],'WFAPJob','APJOB')
    os.chdir(cwd)
    assert Output!=False ,fn_InsertFeatureStepExecutionInfo(EXECUTION_ID,context.feature.name,context.scenario.name,f'Start APJOB','APJOB  Fatal - Assert')
...
def step_impl(context):
    ...
    context.ps_script_path = os.path.join(Configuration["Posting2.0"], Configuration["PSFileName"])
    print(context.ps_script_path)
    try:
        output = subprocess.check_output(['powershell.exe', '-File', context.ps_script_path], stderr=subprocess.STDOUT,
...
def step_impl(context,cnt,amount,trancode,posttime):
    try:
        ...
        if '+' in amount:
            ...
            for amountValue in AmountList:
                if not re.match(r'^\d+$', amountValue):
                    TotalAmount = TotalAmount + float(fn_GetValueFromDataStore(EXECUTION_ID, amountValue))
                else:
        ...
        assert len(context.payment_table[0]) == 2, fn_InsertFeatureStepExecutionInfo(EXECUTION_ID, context.feature.name,
                                                                                    context.scenario.name,
...
def step_impl(context, cnt):
    try:
        ...
        assert len(context.custom_table[0]) == 2, fn_InsertFeatureStepExecutionInfo(EXECUTION_ID, context.feature.name,
                                                                                    context.scenario.name,
                                                                                    "Save tag into variable",
                                                                                    'Fail More than two column')
        try:
...
def step_impl(context):
    try:
        ...
        print(EXECUTION_ID)
        fn_InsertFeatureStepExecutionInfo(EXECUTION_ID, context.feature.name, context.scenario.name,
                                          "Get Account Plan Details", 'InProgress')
        data = {
            "TagName": ["AccountNumber"],
            "VariableName": ["@MyAccountNumber"]
        }
        ...
        # context.AccountNumber = payLoad['AccountNumber']
        fn_InsertFeatureStepExecutionInfo(EXECUTION_ID, context.feature.name, context.scenario.name,
                                          "Get Account Plan Details", 'Done')

        # file_path = f'E:\\Python\\BehaveBDD\\features\\JsonPayload\\PlanDetails_{context.feature.name}.json'
        file_path = f'Account_Plan_Details_{context.feature.name}_{context.scenario.name}.json'
        # JsonData = context.response.json()
        # context.jsonData = context.response.json()
