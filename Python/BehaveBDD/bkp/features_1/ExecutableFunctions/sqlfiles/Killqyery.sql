...
where dbid in(	select database_id 
	from sys.databases where name like 'PP_%')
...
select * FROM MASTER..SysProcesses WHERE DBId in (select database_id from sys.databases where name like 'PP_%') AND SPId <> @@SPId
