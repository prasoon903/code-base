...
@VariableName VARCHAR(100)
AS
DECLARE @VariableValue Decimal(19,0)
SELECT @VariableValue= VariableValue  FROM FeatureStepDataStore WITH(NOLOCK) 
...
BEGIN
	...
	where FeatureName = @FeatureName AND VariableName=@VariableName
END
ELSE
BEGIN
	select @VariableValue = VariableValue from FeatureStepDataStore (nolock)
...
as
(
    select accountnumber,tranid, txncode_internal, cmttrantype, posttime, trantime, transactionamount from ccard_primary(nolock) where tranid = @tranid

)
select accountnumber,
cast(tranid as varchar(50)) tranid, txncode_internal, cmttrantype, CONVERT(varchar, posttime, 120)
posttime, CONVERT(varchar, trantime, 120)
trantime, cast(transactionamount as varchar) transactionamount, M.ActualTrancode
, M.ManualAdjustmentTxnCode, M1.ActualTrancode as ManualReversalTrancode
from CTE Join
MonetaryTxnControl M(nolock) on CTE.txncode_internal = M.TransactionCode
Join MonetaryTxnControl M1(nolock) on
M1.TransactionCode = M.ManualAdjustmentTxnCode
