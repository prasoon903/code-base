...
#######################################################################################################################            
def DBRestore():
    MessageLogger.info("*************************************Database Restoration*****************************************")
    for key , value in YourDBNames.items():
    ...
    MessageLogger.info(Fore.GREEN+"DB Restored Successfully")
    time.sleep(5)
...
def DBRestoreWithBackUp():
    ...
    for key , value in YourDBNames.items():
        ...
        if res == 1 and bproceed == False:
            MessageLogger.info("Error while Restoring "+value )
            exit()
    MessageLogger.info(Fore.GREEN+"DB Restored with Backup Successfully")
