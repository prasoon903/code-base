...
for _, row in df.iterrows():
    ...
    for i in range((to_date - from_date).days // 30 + 2):
        ...
        for j in range(total_bills_per_month):
            ...
            data = {
                    "Month": current_month_new.strftime("%B"),
                    "Date": current_month_new.strftime('%Y-%m-%d'),
                    "Amount": bill_amount,
                    "type": vtype
                    }
            # billData = {f"{receipt_no}": data}
            billData[f"{receipt_no}"] = data
...
# print(f"Billdata: {billData}")
df1 = pd.DataFrame.from_dict(billData, orient='index')

# Write DataFrame to Excel file
output_file = 'output_data.xlsx'
df1.to_excel(os.path.join(pdf_folder, output_file), index_label='BillID')
