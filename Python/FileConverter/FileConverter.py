...
def DirectoryValidation(InputDir, OutputDir, ErrorDir, LogDir, ResponseFileDir, DumpedFileDir, SQLDir):
    ErrorFlag = True
    ErrorNumber = 0
    Message = ""
    if InputDir == "":
...
def CallSQLScriptsFromFolder(SQLDir, SERVER, DB):
    MessageLogger.info("Running CallSQLScripts from FolderName: " + str(SQLDir))
...
if __name__ == '__main__':
    ...
    InputDir, OutputDir, ErrorDir, LogDir, ResponseFileDir, DumpedFileDir, SQLDir, Error = DirectoryValidation(InputDir, OutputDir, ErrorDir, LogDir, ResponseFileDir, DumpedFileDir, SQLDir)
