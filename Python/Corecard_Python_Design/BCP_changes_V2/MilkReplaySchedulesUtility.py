...
RetailAWSEnvironment = int(Config['DEFAULT']['AWSEnvironment'])
...
RetailAWS_service_name = Config['DEFAULT']['ses_service_name']
...
RetailGeneratedFile = Config['MilkReplaySchedulesUtility']['RetailGeneratedFile']
...
def printVaribales(MessageLogger):
    ...
    MessageLogger.info("RetailGeneratedFile: " + str(RetailGeneratedFile))
...
def DirectoryValidation():
    ...
    elif RetailGeneratedFile == "":
        ErrorFlag = True
        Message = "Environment variable for Generated folder is not set"
    else:
        ErrorFlag = False
    ...
    if ErrorFlag == False:
        ...
        GenDir = RetailGeneratedFile + "\\"

    return InputDir, OutputDir, ErrorDir, LogDir, GenDir, ErrorFlag 
...
def DataValidation(FileID, FileName, ChildFileName, MessageLogger):
    ...
    try:
        ...
        if ChildFileName.find("_PART2") != -1:
            FileType = 3
...
def GenerateExceptionFile(FileID, FileName, ExceptionFile, MessageLogger):
    ...
    try:
        ...
        if ConnectionStatus:
            ...
            ResultDataFrame['AccountNumber'] = ResultDataFrame['AccountNumber'].str.replace(r'\r', '')
            ...
            if TotalRecords > 0:
                ResultDataFrame.to_csv(ExceptionFileName, sep=',', index=False, mode='a', escapechar='\n', quotechar='"'  )
...
def readOutputBCPDetails(LogDir, Part, MessageLogger):
    MessageLogger.info("Reading BCP Output file")
    OutputFile = LogDir + "Output" + str(Part) + "_in.log"
    Count = 0
...
def ImportFileToDB(InputDir, FileName, dir_path, LogDir, MessageLogger):
    ...
    try:
        ...
        if ConnectionStatus:
            ...
            for PartFile in FileName:
                MessageLogger.info("IMPORTING FILE DATA OF FILE : " + PartFile)
                File = InputDir + PartFile
                Part = 0

                if Error is False:
                    if PartFile.find("_PART1") != -1 :
                    ...
                    if PartFile.find("_PART2") != -1 :
                        BCP = "bcp " + CI_DB + "..ILPScheduleDetailsBAD_DUMP IN " + File + " -b 5000 -h TABLOCK " \
                                "-f " + dir_path + "MilkReplaySchedules2.xml -e " + str(LogDir) + "Error2_in.log -o "  \
                                + str(LogDir) + "Output2_in.log -S " + SERVERNAME + " -T -t , -F 2"
                        Part = 2
                ...
                try:
                    ...
                    subprocess.run(BCP)
                    readOutputBCPDetails(LogDir, Part, MessageLogger)
                    Error = False
                    ErrorMessage = "NO ERROR"
    ...
    with open(file_name, 'r') as read_obj:
        ...
        for line in read_obj:
            ...
            if string_to_search in line:
                ...
                list_of_negative_results.append((line_number, line.rstrip() ))
    # Return list of tuples containing line numbers and lines where string is found
    return list_of_results, list_of_negative_results 

def GenerateErrorPotenialFile(GenDir, InputDir, FileName, MessageLogger):
    MessageLogger.info("IN GenerateErrorPotenialFile FUNCTION ")
    ErrorPotenial = []
    NoErrorPotenial = []
    genFile = ''
    AllGenFile = []

    if FileName.find("Gen_") == -1:
        try:
            matched_lines, Non_matched_lines = search_string_in_file( InputDir + FileName, '"')
            FileColumnStructure = 'Report_Date,business_date,batch_timestamp,account_uuid,plan_uuid,schedule_id,institution_id,Product_ID,error,error_message,"field_path",AccountNumber\n'
            ErrorPotenial.append(FileColumnStructure)
            for elem in matched_lines:
                ErrorPotenial.append(elem[1])
                ErrorPotenial.append("\n")
            
            for elem in Non_matched_lines:
                NoErrorPotenial.append(elem[1])
                NoErrorPotenial.append("\n")

            genFile = "Gen_" + FileName[:-4] + "_PART2" + ".csv"
            file1 = open( GenDir + genFile, "w")
            file1.writelines(ErrorPotenial)
            file1.close() 

            AllGenFile.append(genFile)

            genFile = "Gen_" + FileName[:-4] + "_PART1" + ".csv"
            file1 = open( GenDir + genFile, "w")
            file1.writelines(NoErrorPotenial)
            file1.close() 

            AllGenFile.append(genFile)

        except Exception as e:
            MessageLogger.error("ERROR IN GenerateErrorPotenialFile BLOCK " + str(e))
    else:
        MessageLogger.info("Already Processing Generated File")
...
InputDir, OutputDir, ErrorDir, LogDir, GenDir, ABORTPROCESSING = DirectoryValidation()
GenOutputDir = OutputDir + "GENERATED\\"
...
if ABORTPROCESSING is False:
    ...
    try:
        for File in glob.glob("*"):
            FinalSuccessfulRecordsCount = 0
            FinalErrorRecordCount = 0
            FinalTotalRecords = 0
            FileName = File
            ...
            if ABORTPROCESSING is False:
                GeneratedFileNames = GenerateErrorPotenialFile(GenDir, InputDir, FileName, MessageLogger)

            if ABORTPROCESSING is False:
                ABORTPROCESSING, ErrorReason = ImportFileToDB(GenDir, GeneratedFileNames, dir_path, LogDir, MessageLogger)
                for GenFileName in GeneratedFileNames:
                    if ABORTPROCESSING:
                        MoveFileToError(GenDir, ErrorDir, GenFileName)

                if ABORTPROCESSING is False:
                    ABORTPROCESSING, ErrorFlag, ErrorReason, TotalRecords, SuccessfulRecordsCount, ErrorRecordCount, FileDetailsEmailBody, ExceptionFile = DataValidation(FileNameId, FileName, GenFileName, MessageLogger)
                    FinalTotalRecords = FinalTotalRecords + TotalRecords
                    FinalSuccessfulRecordsCount = FinalSuccessfulRecordsCount + SuccessfulRecordsCount
                    FinalErrorRecordCount = FinalErrorRecordCount + ErrorRecordCount

                for GenFileName in GeneratedFileNames:
                    if ABORTPROCESSING:
                        MoveFileToError(GenDir, ErrorDir, GenFileName)
                    else:
                        MoveFileToOutput(GenDir, GenOutputDir, GenFileName)
            ...
            if ABORTPROCESSING is False:
                EmailBody = EmailBody + EmailBody_Success(FileName, FinalTotalRecords, FinalSuccessfulRecordsCount, FinalErrorRecordCount, ErrorReason, MessageLogger)
            else:
                EmailBody = EmailBody + EmailBody_Error(FileName, FinalTotalRecords, FinalSuccessfulRecordsCount, FinalErrorRecordCount, ErrorReason, MessageLogger)
